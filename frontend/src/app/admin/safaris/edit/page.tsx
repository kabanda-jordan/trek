...
function EditSafariForm() {
    ...
    api
      ...
      .then((s) =>
        setForm({
          ...
          durationNights: s.durationNights || 0,
          price: s.price?.toString() || "",
          maxParticipants: s.maxParticipants || 10,
          difficultyLevel: s.difficultyLevel || "MODERATE",
      ...
      .catch(() => alert("Failed to load safari"))
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await api.put(`/admin/safaris/${id}`, {
        ...form,
        price: Number(form.price),
      });
      router.push("/admin/safaris");
    } catch (err: any) {
      alert(err.message || "Failed to update safari");
    } finally {
      setSaving(false);
    }
  };

  if (!id) return <div className="p-8 text-center text-slate-500">No safari ID provided</div>;
  if (loading) return <div className="p-8 text-center text-slate-400">Loading...</div>;

  return (
    <>
      <PageHeader title="Edit Safari" description="Update safari details" />
      <Card>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6 max-w-2xl">
            ...
            <div>
              ...
              <textarea rows={4} value={form.itinerary} onChange={(e) => set("itinerary", e.target.value)} className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500" />
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Included Items</label>
                <textarea rows={3} value={form.includedItems} onChange={(e) => set("includedItems", e.target.value)} className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500" />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Excluded Items</label>
                <textarea rows={3} value={form.excludedItems} onChange={(e) => set("excludedItems", e.target.value)} className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500" />
              </div>
            </div>
...
export default function AdminSafarisEditPage() {
  return (
